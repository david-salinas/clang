...
/// the fat binary is built by embedding each of the device images into the
/// host. The linker script also defines a few symbols required by the code
/// generation so that the images can be easily retrieved at runtime. This
/// should be used only in tool chains that support linker scripts.
void tools::AddHIPLinkerScript(const ToolChain &TC, Compilation &C,
  ...
  for (const auto &II : Inputs) {
    ...
    if (A && isa<LinkJobAction>(A) && A->isDeviceOffloading(Action::OFK_HIP)) {
      BundlerTargetArg = BundlerTargetArg + ",openmp-amdgcn--amdhsa-" +
